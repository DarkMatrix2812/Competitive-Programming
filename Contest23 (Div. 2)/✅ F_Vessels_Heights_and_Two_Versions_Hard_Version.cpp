...
void solve()
{
    ...
    for (int i = 1; i < n; i ++) 
    {
        tmp.push_back(h[(idx + i) % n]);
    }
    vector<int> ans(n);
    vector<int> pref_max(n), suff_max(n);
    stack<int> prefh, suffh, prefc, suffc;
    int sum = 0;
    for (int i = 1; i < n; i ++)
    {
        int cnt = 1;
        while (!prefh.empty())
        {
            if (prefh.top() <= tmp[i - 1])
            {
                sum -= (prefc.top() * prefh.top());
                cnt += prefc.top();
                prefh.pop();
                prefc.pop();
            }
    ...
    for (int i = n - 1; i >= 1; i --)
    {
        ...
        while (!suffh.empty())
        {
            if (suffh.top() <= tmp[i - 1])
            {
                sum -= (suffc.top() * suffh.top());
                cnt += suffc.top();
                suffh.pop();
                suffc.pop();
            }
    ...
    for (int i = 0; i < n; i ++)
    {
        ans[(i + idx + 1) % n] = (pref_max[i] + suff_max[i]); 
    }
    for (int x : ans)
    {
        cout << x << " ";

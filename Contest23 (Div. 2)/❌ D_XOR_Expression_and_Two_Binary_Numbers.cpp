...
void solve()
{
    // GENERATIONAL FUMBLE I WAS USING K INSTEAD OF 2^K + 1 FAAAHHHHH
    int n, k;
    cin >> n >> k;
    string s, z;
    cin >> s >> z;
    // only a_1, a_(2^k + 1) and their xor appear finally
    ...
    for (int i = 0; i < n; i ++)
    {
        if (s[i] == '1' && z[i] == '0') cnt3 += 1;
        else if (s[i] == '0' && z[i] == '1') cnt3 += 1;
    }
    int ans1 = cnt1 * (n - cnt1);
    int ans2 = cnt2 * (n - cnt2);
    int ans3 = cnt3 * (n - cnt3);
    if (k % 2 == 1) cout << ((1LL << k) + 1) / 3 * (ans1 + ans2 + ans3) << endl;
    else cout << ((((1LL << k) + 1) + 1) / 3) * (ans1 + ans2) + ans3 * (((1LL << k) + 1) - 2) / 3 << endl;
}
int32_t main() 
{

...
void solve()
{
    int n, m, q;
    cin >> n >> m >> q;
    vector<bool> seena(4e5 + 5, false);
    vector<bool> seenb(4e5 + 5, false);
    vector<int> a(n + 1);
    int sa = 0;
    for (int i = 1; i <= n; i ++)
    {
    ...
    unordered_set<int> tmpa;
    for (int i = 1; i <= n; i ++)
    {
        if (abs(sa - a[i]) <= 2e5) seena[sa - a[i] + 2e5] = true;
    }
    vector<int> b(m + 1);
    int sb = 0;
    for (int i = 1; i <= m; i ++)
    ...
    unordered_set<int> tmpb;
    for (int i = 1; i <= m; i ++)
    {
       if (abs(sb - b[i]) <= 2e5) seenb[sb - b[i] + 2e5] = true;
    }
    for (int i = 0; i < q; i ++)
    {
        int x;
        ...
        if (x == 0)
        {
            ok = (seena[2e5] && m > 0) || (seenb[2e5] && n > 0);
        }
        else
        {
            for (int d = 1; d * d <= abs(x); d ++)
            {
                if (abs(x) % d == 0)
                {
                    ...
                    if (x > 0)
                    {
                        if ((seena[d + 2e5] && seenb[other + 2e5]) || 
                            (seena[other + 2e5] && seenb[d + 2e5]) ||
                            (seena[-d + 2e5] && seenb[-other + 2e5]) || 
                            (seena[-other + 2e5] && seenb[-d + 2e5]))
                        {
                            ok = true;
                            break;

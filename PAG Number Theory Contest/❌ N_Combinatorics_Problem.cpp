...
void solve()
{
    ...
    for (int i = 2; i <= n; i ++)
    {
        pref[i] = (pref[i - 1] + a[i]) % MOD2;
    }
    // b_i = dp[i][k] = dp[i - 1][k - 1] + dp[i - 1][k] + 1Ck * a_i
    // dp[i][0] = a_1 + a_2 + ... + a_i
    ...
    if (k == 1)
    {
        for (int i = 1; i <= n; i ++)
        {
            dp[i][k] = (dp[i - 1][k - 1] + dp[i - 1][k] + a[i]) % MOD2;
        }
        int ans = dp[1][k];
        for (int i = 2; i <= n; i ++)
        {
    ...
    else
    {
        for (int i = 1; i <= n; i ++)
        {
            dp[i][1] = (dp[i - 1][0] + dp[i - 1][1] + a[i]) % MOD2;
        }
        for (int j = 2; j <= k; j ++)
        {
            for (int i = 1; i <= n; i ++)
            {
                dp[i][j] = (dp[i - 1][j - 1] + dp[i - 1][j]) % MOD2;
            }
        }
        int ans = dp[1][k];
        for (int i = 2; i <= n; i ++)

...
int ans(int n)
{
    ...
    {
        int threshold = s[i] - '0';
        for (int d = 0; d < threshold; d ++)
        {
            int tmp = nonzero + (d > 0 ? 1 : 0);
            if (tmp <= 3)
            {
                int rem = len - i - 1;
                for (int places = 0; places <= 3 - tmp; places ++)
                {
                    if (places == 0) result += 1;
                    else if (places == 1) result += rem * 9;
                    else if (places == 2) result += (rem * (rem - 1) / 2) * 9 * 9;
                    else result += (rem * (rem - 1) * (rem - 2) / 6) * 9 * 9 * 9;
                }
            }
        }
        if (threshold > 0)

...
void solve()
{
    ...
    int n;
    cin >> n;
    // find one endpoint => n queries
    // second endpoint => n queries?
    int end1 = 1;
    int d = 0;
    ...
    for (int node = 2; node <= n; node ++)
    {
        if (node == end1) continue;
        while (true)
        {
            cout << "? " << end1 << " " << node << " " << diameter + 1 << endl;
            int x;
            cin >> x;
            if (x == 1)
            {

...
void solve()
{
    ...
    cin >> a >> b >> x;
    double pi = acos(-1);
    double angle1 = atan((2.0 * x) / (a * b * b)) * 180.0 / pi;
    double angle2 = atan((a * a * a) / (2.0 * (a * a * b - x))) * 180.0 / pi;
    if (x <= (a * a * b) / 2.0) cout << fixed << setprecision(6) << 90 - angle1 << endl;
    else cout << fixed << setprecision(6) << 90 - angle2 << endl;
}
int32_t main() 
{

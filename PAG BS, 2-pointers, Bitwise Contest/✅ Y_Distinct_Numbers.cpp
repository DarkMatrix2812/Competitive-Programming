...
void solve()
{
    ...
    sort(f.begin(), f.end());
    vector<int> pref = prefixArr(f);
    for (int k = 2; k <= freq.size(); k ++)
    {
        int l = 1; int r = n; int ans = 0; bool ok = false;
        while (l <= r)
        {
            int mid = (l + r) / 2;
            int idx = lower_bound(f.begin(), f.end(), mid) - f.begin();
            if ((pref[idx] + (f.size() - idx) * mid) >= k * mid)
            {
                ans = mid;
                l = mid + 1;
            }

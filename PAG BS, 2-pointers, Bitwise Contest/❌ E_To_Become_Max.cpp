...
// // clear k-th bit
// x & ~(1LL << k);
bool check(int x, int n, int k, vector<int> a) // check if we can make any element >= x
{
    for (int i = 0; i < n; i ++)
    {
        int ops = 0;
        int target = x; bool ok = true;
        for (int j = i; j < n; j ++)
        {
            if (target <= a[j]) break; // already satisfied
            if (j == n - 1)
            {
                ok = false;
                break;
...
void solve()
{
    ...
    int low = *max_element(a.begin(), a.end()); int high = low + k; int ans = 0;
    while (low <= high)
    {

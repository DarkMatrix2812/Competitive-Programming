...
void solve()
{
    ...
    // for a certain k, eventually we will reach M, then for all k after this ans is just M
    ...
    for (int x : k)
    {
        auto it = lower_bound(consecutive.begin(), consecutive.end(), x);
        if (it != consecutive.end()) cout << st[*it] - 1 << " ";
        else cout << m << " ";
    }
    cout << endl;
}

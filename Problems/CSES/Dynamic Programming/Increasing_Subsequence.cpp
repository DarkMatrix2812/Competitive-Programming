...
{
    for (; i <= bit_n; i += i & -i) bit_arr[i] = max(bit_arr[i], val);
}
int fenwick_query(int i) 
{
    int mx = 0;
    for (; i > 0; i -= i & -i) mx = max(mx, bit_arr[i]);
    return mx;
}
...
void solve()
{
    ...
    for (int &v : x)
    {
        v = lower_bound(vals.begin(), vals.end(), v) - vals.begin() + 1;
    }
    ...
    for (int i = 0; i < n; i ++)
    {
        dp[i] = fenwick_query(x[i] - 1) + 1;
        fenwick_update(x[i], dp[i]);
    }

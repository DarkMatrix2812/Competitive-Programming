...
void solve()
{
    ...
    if (p >= 3) // then the max(x + y + z) not in array
    {
        cout << "NO" << endl;
        return;
    }
    if (neg >= 3) // then the min(x + y + z) not in array
    {
        cout << "NO" << endl;
        return;
    }
    if (z == 0) // brute force it
    {
        for (int i = 0; i < n; i ++)
        {
            for (int j = i + 1; j < n; j ++)
    ...
    else // we just need one zero to brute force
    {

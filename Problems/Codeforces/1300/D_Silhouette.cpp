...
void solve()
{
    ...
    cin >> n;
    vector<int> b(n);
    map<int, int> freq; // unordered_map caused TLE on system testing
    map<int, int> ans; // unordered_map caused TLE on system testing
    for (int i = 0; i < n; i ++)
    {
        cin >> b[i];
        freq[b[i]] += 1;
    ...
    {
        if ((tmp[i] - tmp[i - 1]) % freq[tmp[i - 1]] != 0) 
        {   
            cout << -1 << endl;
            return;
        }
        else ans[tmp[i - 1]] = (tmp[i] - tmp[i - 1]) / freq[tmp[i - 1]];
    }
    for (int i = 1; i < tmp.size() - 1; i++) 
    {
        if (ans[tmp[i - 1]] >= ans[tmp[i]]) 
        {
            cout << -1 << endl;
            return;
        }
    }
    ans[tmp.back()] = ans[tmp[tmp.size() - 2]] + 1;
    for (int i = 0; i < n; i ++)
    {
        cout << ans[b[i]] << " ";
    }

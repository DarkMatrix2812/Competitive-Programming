...
void solve()
{
    ...
    {
        cin >> h[i] >> m[i];
        time.push_back({h[i], m[i]});
    }
    sort(time.begin(), time.end());
    auto it = lower_bound(time.begin(), time.end(), pair<int, int>{H, M});
    if (it == time.end())
    {
        int hours = time[0].first - H + 24;
        int minutes = time[0].second - M;
        if (minutes < 0)
        {
            minutes += 60;
            hours -= 1;
    ...
    {
        int hours = (*it).first - H;
        int minutes = (*it).second - M;
        if (minutes < 0)
        {
            minutes += 60;
            hours -= 1;

...
void solve()
{
    ...
    cin >> n;
    vector<int> a(n);
    vector<__int128> pref_prod(n + 1, 1);
    vector<int> pos;
    for (int i = 0; i < n; i ++)
    {
        cin >> a[i];
        if (a[i] > 1) pos.push_back(i + 1);
        __int128 p = pref_prod[i] * a[i];
        __int128 overflow = (__int128)2000000000000000000LL;
        pref_prod[i + 1] = (p > overflow ? overflow : p);
    }
    vector<int> pref_sum = prefixArr(a);
    if (pref_prod[n] >= (__int128)2000000000000000000LL)
    {
    ...
    else
    {
        ...
        for (int i = 0; i < pos.size(); i ++)
        {
            for (int j = i; j < pos.size(); j ++)
            {
                int l = pos[i];
                int r = pos[j];
                if (pref_prod[r] / pref_prod[l - 1] - pref_sum[r] + pref_sum[l - 1] >= mx)
                {
                    ansl = l;
                    ansr = r;
                    mx = pref_prod[r] / pref_prod[l - 1] - pref_sum[r] + pref_sum[l - 1];
                }
            }
        }
        cout << ansl << " " << ansr << endl;

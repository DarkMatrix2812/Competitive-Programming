...
// // clear k-th bit
// x & ~(1LL << k);
bool check(int median, vector<int> v, int k)
{
    for (int &x : v)
    {
        if (x >= median) x = 1;
    ...
    vector<int> pref = prefixArr(v);
    int mn = pref[0];
    for (int r = k; r < pref.size(); r ++)
    {
        mn = min(mn, pref[r - k]);
        if (pref[r] > mn) return true;
    }
    return false;
}
void solve()
{
    ...
    int low = 0; int high = *max_element(a.begin(), a.end()); int ans = 0;
    while (low <= high)
    {

...
void solve()
{
    ...
    // now sort in optimal way to maximize (r - l) of operation 1
    // sort by l + r
    ...
    if (n % 2 == 0)
    {
        for (int i = 0; i < n / 2; i ++)
        {
            ans += (intervals[n - i - 1].second - intervals[i].first);
        }
        cout << ans << endl;
    }
    else
    {
        ...
        for (int i = 0; i < n / 2; i ++)
        {
            add += (intervals[n - i - 1].second - intervals[i].first);
        }
        mx = max(mx, add);
        for (int i = 0; i < n / 2; i ++)
        {
            mx = max(mx, add + intervals[i].first - intervals[n / 2].first);
        }
        for (int i = n / 2 + 1; i < n; i ++)
        {
            mx = max(mx, add - intervals[i].second + intervals[n / 2].second);
        }
        cout << ans + mx << endl;
    }
}

...
void solve()
{
    ...
    {
        int l = r - 1;
        if (pref[l] >= 0) min_pref = min(min_pref, pref[l]);
        if (min_pref != INT_MAX && pref[r] >= min_pref)
        {
            cout << "YES" << endl;
            return;
        }
    ...
    {
        int l = r - 1;
        if (pref[n] >= pref[r] && pref[r] >= pref_min[l])
        {
            cout << "YES" << endl;
            return;
        }
    ...
    for (int r = 2; r < n; r++)
    {
        if (first_valid != -1 && r > first_valid && pref[n] >= pref[r])
        {
            cout << "YES" << endl;
            return;
        }

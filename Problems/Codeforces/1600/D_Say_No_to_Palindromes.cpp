...
void solve()
{
    ...
    string s;
    cin >> s;
    // palindrome must be of the form abcabc..., bcabca..., cabcab..., etc
    char curr = 'a';
    string t = "";
    while (t.length() != n)
    {
    ...
    for (int i = 0; i < n; i ++)
    {
        if (s[i] != t[i]) p.push_back(1);
        else p.push_back(0);
    }
    vector<int> pref1 = prefixArr(p);
    curr = 'b';
    t = "";
    while (t.length() != n)
    {
    ...
    vector<int> pref6 = prefixArr(p);
    for (int i = 0; i < m; i ++)
    {
        int l, r;
        cin >> l >> r;
        int ans = INT_MAX;
        ans = min({ans, pref1[r] - pref1[l - 1], pref2[r] - pref2[l - 1], pref3[r] - pref3[l - 1], pref4[r] - pref4[l - 1], pref5[r] - pref5[l - 1], pref6[r] - pref6[l - 1]});
        cout << ans << endl;
    }
}
int32_t main() 

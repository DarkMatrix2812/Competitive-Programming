...
void solve()
{
    ...
    int l, r;
    cin >> l >> r;
    // just use 2-multiplier for max set size
    int tmp = l;
    int m = 1;
    while (2 * tmp <= r)
    {
    ...
    cout << m << " ";
    // 2^(k-1) * l <= r < 2^k * l
    // 3 * 2^(k-2) * l <= r < 2^k * l also works
    int ans = 0;
    // for (int i = l; i <= r; i ++)
    // {
    //     if ((1LL << (m - 1)) * i <= r) ans = (ans + 1) % MOD2;
    //     if (3 * (1LL << (m - 2)) * i <= r) ans = (ans + (m - 1)) % MOD2;
    // }
    if (m >= 1) 
    {
        if (r / (1LL << (m - 1)) >= l) ans = (ans + (r / (1LL << (m - 1)) - l + 1)) % MOD2;
    }
    if (m >= 2) 
    {
        if (r / (3 * (1LL << (m - 2))) >= l) ans = (ans + (m - 1) * (r / (3 * (1LL << (m - 2))) - l + 1)) % MOD2;
    }
    cout << ans % MOD2 << endl;
}
int32_t main() 

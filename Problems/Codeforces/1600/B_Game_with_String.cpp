...
void solve()
{
    ...
    string s;
    cin >> s;
    // find all shifts and store by starting letter
    // each column - the max count of elements which has freq = 1
    int n = s.size();
    vector<string> shifts;
    for (int k = 0; k < n; k ++)
    {
        shifts.push_back(s.substr(k) + s.substr(0, k));
    }
    ...
    cout << fixed << setprecision(6) << (double)ans / n << endl;
}
int32_t main() 
{

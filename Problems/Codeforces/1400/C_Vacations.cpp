...
void solve()
{
    ...
    vector<vector<int>> dp(n + 1, vector<int>(3, INT_MAX)); // 0 - rest, 1 - gym, 2 - contest
    dp[0][0] = 0;
    for (int i = 1; i <= n; i ++)
    {
        dp[i][0] = min({dp[i - 1][0], dp[i - 1][1], dp[i - 1][2]}) + 1;
        if (a[i - 1] == 2 || a[i - 1] == 3) dp[i][1] = min(dp[i - 1][0], dp[i - 1][2]);
        if (a[i - 1] == 1 || a[i - 1] == 3) dp[i][2] = min(dp[i - 1][0], dp[i - 1][1]);
    }

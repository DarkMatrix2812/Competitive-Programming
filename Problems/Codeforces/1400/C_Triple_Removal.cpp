...
void solve()
{
    ...
    vector<int> pref1(n + 1), pref0(n + 1);
    pref1[1] = (a[1] == 1);
    pref0[1] = (a[1] == 0);
    for (int i = 2; i <= n; i ++)
    {
    ...
    for (int i = 0; i < q; i ++)
    {
        int l, r;
        cin >> l >> r;
        // we just need to check original [l, r]
        // if it is alternating - that's the only occasion cost is 2
        int flag = 1;
        auto it = s.lower_bound(l);
        if (it != s.end() && *it < r) flag = 0;
        if (((pref1[r] - pref1[l - 1]) % 3 == 0) && ((pref0[r] - pref0[l - 1]) % 3 == 0)) cout << (r - l + 1) / 3 + flag << endl;
        else cout << -1 << endl;
    }
}
int32_t main() 

...
void solve()
{
    ...
    for (int d = l[0]; d <= r[0]; d ++)
    {
        ...
        else furthest = max(furthest, pos[d][0]);
    }
    for (int i = 1; i < m; i ++)
    {
        int best = -1;
        ...
        {
            if (pos[d].empty() || (upper_bound(pos[d].begin(), pos[d].end(), furthest) == pos[d].end()))
            {
                cout << "YES" << endl;
                return;
            }
            best = max(best, *upper_bound(pos[d].begin(), pos[d].end(), furthest));
        }
        furthest = best;
    }
    cout << "NO" << endl;

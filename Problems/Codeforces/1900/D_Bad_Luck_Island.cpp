...
void solve()
{
    ...
    int r, s, p;
    cin >> r >> s >> p;
    vector<vector<vector<double>>> dp1(r + 1, vector<vector<double>>(s + 1, vector<double>(p + 1, 0)));
    vector<vector<vector<double>>> dp2(r + 1, vector<vector<double>>(s + 1, vector<double>(p + 1, 0)));
    vector<vector<vector<double>>> dp3(r + 1, vector<vector<double>>(s + 1, vector<double>(p + 1, 0)));
    // dp1[x][y][z] = probability rocks only surviving
    // dp2[x][y][z] = probability scissors only surviving
    // dp3[x][y][z] = probability paper only surviving
    for (int x = 0; x <= r; x ++)
    {
        for (int y = 0; y <= s; y ++)
        {
            for (int z = 0; z <= p; z ++)
            {
                if (x != 0 && y == 0 && z == 0) dp1[x][y][z] = 1;
                else if (x == 0 && y != 0 && z == 0) dp2[x][y][z] = 1;
                else if (x == 0 && y == 0 && z != 0) dp3[x][y][z] = 1;
                else
                {
                    if (x != 0 && y != 0)
                    {
                        dp1[x][y][z] += double(x * y) / (x * y + y * z + z * x) * dp1[x][y - 1][z];
                        dp2[x][y][z] += double(x * y) / (x * y + y * z + z * x) * dp2[x][y - 1][z];
                        dp3[x][y][z] += double(x * y) / (x * y + y * z + z * x) * dp3[x][y - 1][z];
                    }
                    if (y != 0 && z != 0)
                    {
                        dp1[x][y][z] += double(y * z) / (x * y + y * z + z * x) * dp1[x][y][z - 1];
                        dp2[x][y][z] += double(y * z) / (x * y + y * z + z * x) * dp2[x][y][z - 1];
                        dp3[x][y][z] += double(y * z) / (x * y + y * z + z * x) * dp3[x][y][z - 1];
                    }
                    if (z != 0 && x != 0)
                    {
                        dp1[x][y][z] += double(z * x) / (x * y + y * z + z * x) * dp1[x - 1][y][z];
                        dp2[x][y][z] += double(z * x) / (x * y + y * z + z * x) * dp2[x - 1][y][z];
                        dp3[x][y][z] += double(z * x) / (x * y + y * z + z * x) * dp3[x - 1][y][z];
                    }
                }
            }
        }
    }
    cout << fixed << setprecision(9) << dp1[r][s][p] << " " << dp2[r][s][p] << " " << dp3[r][s][p];
}
int32_t main() 
{

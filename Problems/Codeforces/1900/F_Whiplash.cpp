...
void solve()
{
    ...
    // a[0] = smallest positive element in b 
    int n;
    cin >> n;
    vector<int> a(n), b(n);
    int Sa = 0;
    ...
    vector<int> states_a, states_b;
    states_a.push_back(Sa);
    states_b.push_back(Sb);
    for (int i = 0; i < n; i ++)
    {
        states_a.push_back(Sa ^ a[i]);
        states_b.push_back(Sb ^ b[i]);
    }
    sort(states_a.begin(), states_a.end());
    sort(states_b.begin(), states_b.end());
    if (states_a == states_b) cout << "YES" << endl;
    else cout << "NO" << endl;
}
int32_t main() 
{

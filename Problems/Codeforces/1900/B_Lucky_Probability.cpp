...
void solve()
{
    ...
    int pl, pr, vl, vr, k;
    cin >> pl >> pr >> vl >> vr >> k;
    // brute force to find all ranges which have k lucky numbers
    int l = min(pl, vl);
    int r = max(pr, vr);
    ...
    vector<int> lucky;
    lucky.push_back(0);
    lucky.push_back(2000000000LL);
    for (int len = max(1LL, dl - 1); len <= dr + 1; len ++) 
    {
    ...
    {
        valid += count(lucky[i - 1] + 1, lucky[i], pl, pr) * count(lucky[i + k - 1], lucky[i + k] - 1, vl, vr);
        valid += count(lucky[i - 1] + 1, lucky[i], vl, vr) * count(lucky[i + k - 1], lucky[i + k] - 1, pl, pr);
        if (k == 1) 
        {
            if (pl <= lucky[i] && lucky[i] <= pr && vl <= lucky[i] && lucky[i] <= vr) valid -= 1;
        }
    }
    cout << fixed << setprecision(9) << (double)valid / ((pr - pl + 1) * (vr - vl + 1));
}
int32_t main() 
{

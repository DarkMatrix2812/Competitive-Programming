...
int binExp(int base, int exp, int M) 
{
    ...
    while (exp > 0) 
    {
        if (exp % 2 == 1) result = ((__int128)result * base) % M;
...
void build_fact(int N, int M) 
{
    ...
    for (int i = N - 1; i >= 0; i--)
    {
        invfact[i] = (__int128)invfact[i + 1] * (i + 1) % M;
    }
}
int nCr(int n, int r, int M) 
{
    if (r < 0 || r > n) return 0;
    int ans = (__int128)fact[n] * invfact[r] % M;
    ans = (__int128)ans * invfact[n - r] % M;
    return ans;
}
...
void solve()
{
    ...
    int p;
    cin >> p;
    // we iterate over k from 1 to n instead of all n! orders
    ...
    for (int k = 1; k <= n; k++) 
    {
        for (int w = 0; w <= p; w++) 
        {
            ans += (double)dp[k][w] / nCr(n, k, 1000000000000000003LL);
        }
    }
...
int32_t main() 
{
    ...
    cin.tie(nullptr);
    int t = 1;
    build_fact(55, 1000000000000000003LL);
    while (t--)
    {
        solve();
    }

...
void solve()
{
    ...
    vector<int> alternatingb(m + 1, 0);
    alternatingb[1] = b[1];
    for (int i = 2; i <= m; i ++)
    {
        if (i % 2 == 0) alternatingb[i] = alternatingb[i - 1] - b[i];
        else alternatingb[i] = alternatingb[i - 1] + b[i];
    }
    multiset<int> Sb;
    for (int j = 0; j <= m - n; j ++)
    {
        if (j % 2 == 0) Sb.insert(alternatingb[j + n] - alternatingb[j]);
        else Sb.insert(alternatingb[j] - alternatingb[j + n]);
    }
    int ans = LLONG_MAX;
    for (int x : Sb)
    {
        ans = min(ans, abs(Sa - x));
    }
    cout << ans << endl;
    for (int i = 0; i < q; i ++)
    {
        int l, r, x;
        cin >> l >> r >> x;
        // if range size is even => alternating sum of a = S is unchanged
        // if range size is odd => starting index is odd: S += x else S -= x
        int ans = LLONG_MAX;
        if ((r - l + 1) % 2 == 0)
        {
            auto it1 = Sb.lower_bound(Sa);
            if (it1 != Sb.end())
            {
                ans = min(ans, abs(Sa - *it1));
            }
            auto it2 = Sb.lower_bound(Sa);
            if (it2 != Sb.begin())
            {
                ans = min(ans, abs(Sa - *--it2));
            }
            cout << ans << endl;
        }
        else

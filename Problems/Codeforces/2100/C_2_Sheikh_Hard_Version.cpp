...
void solve()
{
    ...
    for (int i = 0; i < q; i ++)
    {
        int l, r;
        cin >> l >> r;
        int v = pref[r] - pref[l - 1] - (prefxor[r] ^ prefxor[l - 1]); // this is the maximum value
        if (v == 0) 
        {
            cout << l << " " << l << endl;
            continue;
        ...
        vector<int> left;
        int curr_L = l;
        while (curr_L <= r && left.size() <= 31) 
        {
            left.push_back(curr_L);
            if (a[curr_L - 1] == 0) curr_L = nxt[curr_L]; 
            else curr_L = nxt[curr_L + 1];
        }
        vector<int> right;
        int curr_R = r;
        while (curr_R >= l && right.size() <= 31) 
        {
            right.push_back(curr_R);
            if (a[curr_R - 1] == 0) curr_R = prv[curr_R]; 
            else curr_R = prv[curr_R - 1];               
        }
        ...
        for (int L : left) 
        {
            for (int R : right) 
            {
                if (L <= R) 
                {
                    if (pref[R] - pref[L - 1] - (prefxor[R] ^ prefxor[L - 1]) == v) 
                    {

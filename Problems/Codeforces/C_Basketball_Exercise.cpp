...
void solve()
{
    ...
    vector<int> dp1(n); // dp[i] = max score starting at index i and first row
    vector<int> dp2(n); // dp[i] = max score starting at index i and second row
    dp1[n - 1] = h1[n - 1];
    dp2[n - 1] = h2[n - 1];
    for (int i = n - 2; i >= 0; i --)
    {
        dp1[i] = max(dp1[i + 1], h1[i] + dp2[i + 1]);
        dp2[i] = max(dp2[i + 1], h2[i] + dp1[i + 1]);
    }

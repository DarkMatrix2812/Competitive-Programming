...
void solve()
{
    ...
    int curr_l = intervals[0].first.first; int curr_r = intervals[0].first.second;
    int f = intervals[0].second;
    for (int i = 1; i < intervals.size(); i ++)
    {
        if (intervals[i].first.first >= curr_l && intervals[i].first.second <= curr_r)
        {
            cout << intervals[i].second << " " << f;
            return;
        }
        else
        {
            curr_l = intervals[i].first.first;
            curr_r = intervals[i].first.second;

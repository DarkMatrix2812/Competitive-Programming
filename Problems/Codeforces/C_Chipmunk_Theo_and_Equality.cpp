...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        int curr = a[i];
        int steps = 0;
        if (curr == 1) reachable.push_back({2, 1});
        while (curr > 1)
        {
            reachable.push_back({curr, steps});
            if (curr % 2 == 1) curr += 1;
            else curr /= 2;
            steps += 1;
        }
        reachable.push_back({1, steps});
    }
    sort(reachable.begin(), reachable.end());
    int ans = INT_MAX;
    int curr = reachable[0].first;
    int sz = 1;
    int s = reachable[0].second;
    for (int i = 1; i < reachable.size(); i ++)
    {
        if (reachable[i].first == curr)
        {
            sz += 1;
            s += reachable[i].second;
        }
        else
        {
            if (sz == n) ans = min(ans, s);
            curr = reachable[i].first;
            sz = 1;
            s = reachable[i].second;
        }
        if (sz == n) ans = min(ans, s);

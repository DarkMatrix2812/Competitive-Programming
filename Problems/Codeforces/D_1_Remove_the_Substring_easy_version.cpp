...
void solve()
{
    ...
    for (int l = 0; l < s.length(); l ++)
    {
        ...
        {
            string rem = "";
            for (int i = 0; i < l; i ++) rem.push_back(s[i]);
            for (int j = r + 1; j < s.length(); j ++) rem.push_back(s[j]);
            int i = 0; int j = 0;
            while (i < t.length() && j < rem.length())
            {
                if (t[i] == rem[j])
                {
                    i += 1;
            ...
            if (i == t.length()) ans = max(ans, r - l + 1);
        }
    }
    cout << ans << endl;
}

    ...
    for (int i = 0; i < n; i ++)
    {
        if (!vis[i])
        {
            ...
            else
            {
                vector<int> c1;
                string tmp = s;
                tmp[cycle[0]] = (tmp[cycle[0]] == '1' ? '0' : '1');
                tmp[a[cycle[0]]] = (tmp[a[cycle[0]]] == '1' ? '0' : '1');
                c1.push_back(cycle[0] + 1);
                for (int node = 1; node < cycle.size(); node ++)
                {
                    if (tmp[cycle[node]] == '1')
                    {
                        tmp[cycle[node]] = '0';
                        tmp[a[cycle[node]]] = (tmp[a[cycle[node]]] == '1' ? '0' : '1');
                        c1.push_back(cycle[node] + 1);
                    }
                }
                vector<int> c2;
                for (int node = 1; node < cycle.size(); node ++)
                {
                    ...
                    {
                        s[cycle[node]] = '0';
                        s[a[cycle[node]]] = (s[a[cycle[node]]] == '1' ? '0' : '1');
                        c2.push_back(cycle[node] + 1);
                    }
                }
                if (c1.size() < c2.size())
                {

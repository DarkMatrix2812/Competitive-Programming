...
void solve()
{
    ...
    for (auto &p : freq)
    {
        if (p.second >= 2) dup.push_back(p.second);
        if (p.second >= 4) ans += (p.second * (p.second - 1) * (p.second - 2) * (p.second - 3) / 24); // for (a, a, a, a) form
    }
    int sum = 0, sumsq = 0;
    for(int x : dup) // for (a, a, b, b) form
    {
        int c = x * (x - 1) / 2;
        sum += c;
        sumsq += c * c;
    ...
    ans += (sum * sum - sumsq) / 2;
    for (auto &p : freq) // for (a, a, a + 1, a - 2) form
    {
        if (p.second >= 2)
        {
            if (freq.count(p.first + 1) && freq.count(p.first - 2))
            {
                ans += (p.second * (p.second - 1) / 2) * freq[p.first + 1] * freq[p.first - 2];
            }
        }
    }
    cout << ans << endl;

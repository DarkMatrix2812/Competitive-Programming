...
void solve()
{
    ...
    int w, b;
    cin >> w >> b;
    vector<vector<double>> p(w + 1, vector<double>(b + 1, 0)), d(w + 1, vector<double>(b + 1, 0));
    // p[i][j] = probability princess wins from i white and j black state when princess's turn
    // d[i][j] = probability princess wins from i white and j black state when dragon's turn
    p[0][0] = 0;
    d[0][0] = 1;
    for (int i = 1; i <= w; i ++)
    {
    ...
    for (int i = 1; i <= w; i ++)
    {
        for (int j = 1; j <= b; j ++)
        {
            p[i][j] = (double)i / (i + j) + (double)j / (i + j) * d[i][j - 1];
            d[i][j] = (double)j / (i + j) * ((double)i / (i + j - 1) * p[i - 1][j - 1] + (j >= 2 ? (double)(j - 1) / (i + j - 1) * p[i][j - 2] : 0));
        }
    }
    cout << fixed << setprecision(9) << p[w][b] << endl;
}
int32_t main() 
{

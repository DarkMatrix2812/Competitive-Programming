...
void solve()
{
    ...
    int ans = 0;
    for (int C = 0; C <= 4999 * 2; C ++)
    {
        auto it = upper_bound(diff.begin(), diff.end(), C);
        ans += sum[C] * (diff.end() - it);
    }
    cout << fixed << setprecision(6) << (double)(ans) / ((__int128)(n * (n - 1) / 2.0 * n * (n - 1) / 2.0 * n * (n - 1) / 2.0));
}
int32_t main() 
{

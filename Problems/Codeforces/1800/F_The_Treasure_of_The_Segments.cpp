...
void solve()
{
    ...
    // just check each segment - how many other segments have to be deleted
    sort(l.begin(), l.end());
    sort(r.begin(), r.end());
    int ans = n - 1;
    for (int i = 0; i < n; i ++)
    {
        int right = l.end() - upper_bound(l.begin(), l.end(), intervals[i].second);
        int left = lower_bound(r.begin(), r.end(), intervals[i].first) - r.begin();

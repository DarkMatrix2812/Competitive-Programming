...
void solve()
{
    ...
    for (char c : s)
    {
        if (c == '+') pos.push_back(1);
        else if (c == '-') neg.push_back(1);
    }
    if ((int)pos.size() - n * (int)neg.size() > n || n > n * (int)pos.size() - (int)neg.size())
    {
        cout << "Impossible" << endl;
        return;
    }
    cout << "Possible" << endl;
    if ((int)pos.size() - (int)neg.size() < n)
    {
        int need = n - ((int)pos.size() - (int)neg.size());
        for (int &x : pos)
        {
            if (need < n - x) 
            {
    ...
    {
        int need = ((int)pos.size() - (int)neg.size()) - n;
        for (int &x : neg)
        {
            if (need < n - x)
            {

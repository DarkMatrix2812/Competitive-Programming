...
void solve()
{
    ...
    for (int j = 0; j < n; j ++)
    {
        for (int i = 0; i < j; i ++)
        {
            if ((t[j] - t[i]) * (t[j] - t[i]) >= (x[j] - x[i]) * (x[j] - x[i]) + (y[j] - y[i]) * (y[j] - y[i])) dp[j] = max(dp[j], dp[i] + p[j]);
        }
    }
    cout << fixed << setprecision(6) << *max_element(dp.begin(), dp.end()) << endl;
}
int32_t main() 
{

...
void solve()
{
    ...
    if (n == 1)
    {
        if (s == "_" || s == "0" || s == "X") cout << 1 << endl;
        else cout << 0 << endl;
        return;
    }
    int ans = 0;
    vector<string> end0 = {"00", "50"};
    vector<string> end5 = {"25", "75"};
    auto countWays = [&](string ending)
    {
        int ways = 0;
        bool seen = false;
        for (char c : s)
        ...
        for (int X = 0; X <= 9; X++)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                int pos = n - 2 + i;
                int d = ending[i] - '0';
                if (a[pos] != -1 && a[pos] != d) ok = false;
                a[pos] = d;
            }
            if (!ok || a[0] == 0) continue;
            int curr = 1;
            for (int i = 0; i < n; i++)
            {
                if (a[i] == -1 && i == 0) curr *= 9;
                else if (a[i] == -1 && i != 0) curr *= 10;
            }
            ways += curr;
        }
        return ways;
    };
    if (s[n - 1] == '0' || s[n - 1] == '_' || s[n - 1] == 'X')
    {
        ans += (countWays("00") + countWays("50"));
    }
    if (s[n - 1] == '5' || s[n - 1] == '_' || s[n - 1] == 'X')
    {
        ans += (countWays("25") + countWays("75"));
    }
    cout << ans << endl;
}
int32_t main() 

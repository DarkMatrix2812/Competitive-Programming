...
void solve()
{
    ...
    // O(n^2) all [l, r]
    // query check in O(logn) / O(1) => sparse table?
    vector<int> pref(n + 1, 0);
    for (int i = 1; i <= n; i ++)
    {
        if (i % 2 == 1) pref[i] = pref[i - 1] + c[i];
        else pref[i] = pref[i - 1] - c[i];
    }
    vector<vector<int>> prefMin(n + 1, vector<int>(n + 1));
    for (int i = 1; i <= n; i ++)
    {
        prefMin[i][i] = pref[i];
        for (int j = i + 1; j <= n; j++)
        {
            prefMin[i][j] = min(prefMin[i][j - 1], pref[j]);
        }
    }
    int ans = 0;
    for (int i = 1; i <= n; i += 2)
    {
        ...
        {
            int mn = 0;
            if (i + 1 <= j - 1) mn = min(0LL, prefMin[i + 1][j - 1] - pref[i]);
            ans += max(0LL, min(c[i], c[j] - pref[j - 1] + pref[i]) - max({1LL, -mn, 1LL - pref[j - 1] + pref[i]}) + 1);
        }
    }
    cout << ans;
}

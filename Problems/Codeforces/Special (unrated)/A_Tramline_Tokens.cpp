...
void bfs(int s)
{
    ...
    while (!q.empty())
    {
        auto [v, op, cnt] = q.front();
        q.pop();
        for(auto [u, new_op] : adj[v])
        {
            int new_cnt;
            if (new_op == op)
            {
            ...
            if (!vis[u][new_op][new_cnt])
            {
                vis[u][new_op][new_cnt] = true;
                d[u][new_op][new_cnt] = d[v][op][cnt] + 1;
                q.push({u, new_op, new_cnt});
            }
        }
    }
}
...
void solve()
{
    ...
    int n, m, s, t;
    cin >> n >> m >> s >> t;
    d.assign(n + 1, vector<vector<int>>(3, vector<int>(3, -1)));
    vis.assign(n + 1, vector<vector<bool>>(3, vector<bool>(3, false)));
    adj.assign(n + 1, {});
    for (int i = 0; i < m; i ++)
    {
        int a, b, o;
    ...
    bfs(s);
    int ans = INT_MAX;
    if (d[t][1][1] != -1) ans = min(ans, d[t][1][1]);
    if (d[t][1][2] != -1) ans = min(ans, d[t][1][2]);
    if (d[t][2][1] != -1) ans = min(ans, d[t][2][1]);
    if (d[t][2][2] != -1) ans = min(ans, d[t][2][2]);

...
// // clear k-th bit
// x & ~(1LL << k);
int coprimeTill(int x, vector<int> &pf)
{
    int ans = x;
    int m = pf.size();
    for (int mask = 1; mask < (1 << m); mask++)
    {
        ...
        if (bits & 1) ans -= x / prod;
        else ans += x / prod;
    }
    return ans;
}
int coprimeRangeBoth(int L, int R, int a, int b)
{
    vector<int> pa = primeFactors(a);
    vector<int> pb = primeFactors(b);
    set<int> s;
    for (int x : pa) s.insert(x);
    for (int x : pb) s.insert(x);
    vector<int> pf(s.begin(), s.end());
    return coprimeTill(R, pf) - coprimeTill(L - 1, pf);
}
void solve()
{
    ...
    int a, b, l, r;
    cin >> a >> b >> l >> r;
    // co prime to a and not co prime to b
    vector<int> pa = primeFactors(a);
    cout << coprimeTill(r, pa) - coprimeTill(l - 1, pa) - coprimeRangeBoth(l, r, a, b);
}
int32_t main() 
{

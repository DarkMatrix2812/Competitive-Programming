...
void solve()
{
    ...
    vector<vector<int>> dp(n + 1, vector<int>(k, 0)); // dp[i][r] = probability that the sum of first i dice leaves remainder r (using mod inverse)
    // we need 1 / dp[n][0] = expected rolls
    vector<vector<int>> p(n + 1, vector<int>(k, 0)); // p[i][r] = probability i-th dice leaves a remainder r (using mod inverse)
    dp[0][0] = 1;
    for (int r = 1; r < k; r ++)
    {
        dp[0][r] = 0;
    ...
    for (int i = 1; i <= n; i ++)
    {
        ...
        {
            int cnt = x[i] / k;
            for (int rem = x[i] / k * k + 1; rem <= x[i]; rem ++)
            {
                if (rem % k == r) cnt += 1;
            }
            p[i][r] = cnt * modinv(x[i], MOD) % MOD;
        }
    }
    for (int i = 1; i <= n; i ++)
    {
        for (int r = 0; r < k; r ++)
        {
            for (int j = 0; j < k; j ++)
            {
                dp[i][r] = (dp[i][r] + dp[i - 1][j] * p[i][(r - j + k) % k]) % MOD;
            }
        }
    }
    cout << (dp[n][0] == 0 ? -1 : modinv(dp[n][0], MOD));
}
int32_t main() 
{

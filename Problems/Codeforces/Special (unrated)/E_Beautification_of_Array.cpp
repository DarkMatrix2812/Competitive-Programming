...
void solve()
{
    ...
    // key is to minimize the sum of any k elements: we keep incrementing the smallest
    sort(a.begin(), a.end());
    int K = k;
    int lvl = a[0];
    int pref = 1;
    ...
    {
        int diff = a[idx] - lvl;
        __int128 need = (__int128)diff * pref;
        if (need <= K)
        {
            K -= need;
            lvl = a[idx];
    ...
    {
        S1 = (S1 + x) % MOD;
        S2 = (S2 + (x % MOD) * (x % MOD)) % MOD;
    }
    cout << (((S1 * S1) % MOD - S2 + MOD) % MOD * modinv(2, MOD)) % MOD << endl;
}
int32_t main() 
{

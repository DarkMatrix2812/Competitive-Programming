...
void solve()
{
    ...
    cin >> n;
    vector<int> a(n);
    int cnta = 0; int cnte = 0; int cnti = 0; int cnto = 0; int cntu = 0; int con = 0;
    for (int i = 0; i < n; i ++)
    {
        cin >> a[i];
    }
    ...
    for (int i = 0; i < n; i ++)
    {
        char c = s[i];
        if (c == 'a') cnta += a[i];
        else if (c == 'e') cnte += a[i];
        else if (c == 'i') cnti += a[i];
        else if (c == 'o') cnto += a[i];
        else if (c == 'u') cntu += a[i];
        else con += a[i];
    }
    ...
    int ans = 1;
    ans = (ans * (binExp(2, cnta, MOD) - 1)) % MOD;
    ans = (ans * (binExp(2, cnte, MOD) - 1)) % MOD;
    ans = (ans * (binExp(2, cnti, MOD) - 1)) % MOD;
    ans = (ans * (binExp(2, cnto, MOD) - 1)) % MOD;
    ans = (ans * (binExp(2, cntu, MOD) - 1)) % MOD;
    ans = (ans * binExp(2, con, MOD)) % MOD;
    cout << ans << endl;
}
int32_t main() 
{

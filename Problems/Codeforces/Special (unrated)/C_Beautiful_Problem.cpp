...
void solve()
{
    ...
    int ans = (n >= 2) ? 2 : 1;
    vector<int> dp1(40005, 0);
    for (int i = 1; i <= n; i ++)
    {
        int val = a[i];
        if (val >= 1 && dp1[val - 1] > 0) 
        {
            dp1[val] = dp1[val - 1] + 1;
        } 
        else 
    ...
    vector<vector<int>> dp(4e4 + 1, vector<int>(30, 0)); // dp[i][k] = length of largest subsequence ending at val i with last difference x^k
    for (int x = 2; x <= 200; x ++)
    {
        vector<int> pw;
        pw.push_back(1);
        ...
        for (int i = 1; i <= n; i ++)
        {
            ...
            dp[a[i]][0] = 1;
            for (int k = pw.size() - 1; k >= 1; k --)
            {
                if (a[i] - pw[k] >= 0 && dp[a[i] - pw[k]][k - 1] == k)
                {
                    dp[a[i]][k] = max(dp[a[i]][k], dp[a[i] - pw[k]][k - 1] + 1);

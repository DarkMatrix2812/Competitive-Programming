...
void solve()
{
    ...
    // p[i] = probability of winning from position i
    // p[1] = answer
    // x[i] = probability he wins in that round from position i by reaching time machine
    // y[i] = probability he doesn't win or lose in that round from position i and process repeats
    // z[i] = probability he loses in that round from position i by reaching time machine
    // y[i] = 1 - x[i] - z[i]
    // p[i] = x[i] + y[i] * p[i]
    // we just need x[1] / (1 - y[1]) = x[1] / (x[1] + z[1])
    double x = (b.count(n) ? 1.0 : 0);
    double z = (a.count(n) ? 1.0 : 0);
    for (int i = n - 1; i >= 1; i --)
    {
        if (b.count(i)) x += 1.0;
        else if (a.count(i)) z += 1.0;
        else
        {
            x += (x / (n - i));
            z += (z / (n - i));
    ...
    cout << fixed << setprecision(6) << x / (x + z) << endl;
}
int32_t main() 
{

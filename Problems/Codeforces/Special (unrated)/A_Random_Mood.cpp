...
vector<vector<double>> matrixExp(vector<vector<double>> base, int exp) 
{
    vector<vector<double>> result = identityMatrix(base.size());
    while (exp > 0) 
    {
...
void solve()
{
    ...
    A[0][1] = 1;
    A[1][1] = 1;
    vector<vector<double>> result = matrixExp(A, n);
    cout << fixed << setprecision(9) << result[0][0] + result[0][1] * p;
}
int32_t main() 
{

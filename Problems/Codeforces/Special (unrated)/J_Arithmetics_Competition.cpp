...
void solve()
{
    ...
    vector<int> prefa = prefixArr(a);
    vector<int> prefb = prefixArr(b);
    for (int i = 0; i < q; i ++)
    {
        int x, y, z;
        cin >> x >> y >> z;
        int high = min(x, z); int low = max(0LL, z - y); int ans = low;
        while (low <= high)
        {
        ...
        cout << prefa[ans] + prefb[z - ans] << endl;
    }
}
int32_t main() 
{

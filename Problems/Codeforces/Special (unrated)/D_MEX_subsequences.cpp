...
// // clear k-th bit
// x & ~(1LL << k);
int ans(vector<int> &a, int x) // no. of subsequences with mex >= x
{
    if (x > a.size() + 1) return 0;
    if (x == 0) return binExp(2, a.size(), MOD);
    map<int, int> cnt;
    for (int i : a)
    {
        if (i < x) cnt[i] += 1;
    ...
    for (int i = 0; i < x; i ++)
    {
        p = (p * ((binExp(2, cnt[i], MOD) - 1) + MOD) % MOD) % MOD;
        rem -= cnt[i];
    }
    p = (p * binExp(2, rem, MOD)) % MOD;
    return p;
}
void solve()
{
    ...
    cout << (ans(a, l) - ans(a, r + 1) + MOD) % MOD << endl;
}
int32_t main() 
{

...
void solve()
{
    ...
    // capitals can only move like 0, x1, x2, ..., xn bruh
    vector<int> dp(n + 1, 0); // dp[i] = minimum cost from the state where capital at x_i
    for (int i = n - 1; i >= 0; i --)
    {
        dp[i] = min(dp[i + 1] + (x[i + 1] - x[i]) * b + (x[i + 1] - x[i]) * a, b * ((pref[n + 1] - pref[i + 1]) - (n - i) * x[i]));
    }
    cout << dp[0] << endl;
}
int32_t main() 

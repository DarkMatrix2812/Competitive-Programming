...
void solve()
{
    ...
    // just check if a neighbor has been seen before
    map<int, bool> seen;
    map<int, bool> in;
    ...
    for (int i = 0; i < n; i ++)
    {
        ...
        {
            vector<int> tmp;
            for (int j = 0; j < idx[a[i]].size(); )
            {
                int k = j;
                while (k + 1 < idx[a[i]].size() && idx[a[i]][k + 1] == idx[a[i]][k] + 1)
                {
                    k ++;
                }
                if (in[idx[a[i]][k] + 1])
        ...
        v.push_back(idx[a[i]][cnt[a[i]]]);
        in[idx[a[i]][cnt[a[i]]]] = true; 
        cnt[a[i]] += 1;
    }
    int ans = 0;
    for (int i : v)

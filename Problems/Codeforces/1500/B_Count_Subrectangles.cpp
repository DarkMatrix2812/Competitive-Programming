...
void solve()
{
    ...
    vector<int> consecutive_a;
    int curr = (a[0] == 1 ? 1 : 0);
    for (int i = 1; i < n; i ++)
    {
        if (a[i] == 1 && a[i - 1] == 1) curr += 1;
        ...
        {
            if (curr != 0) consecutive_a.push_back(curr);
            curr = 0;
        }
    }
    if (curr != 0) consecutive_a.push_back(curr);
    vector<int> consecutive_b;
    curr = (b[0] == 1 ? 1 : 0);
    for (int i = 1; i < m; i ++)
    {
        if (b[i] == 1 && b[i - 1] == 1) curr += 1;
        ...
        {
            if (curr != 0) consecutive_b.push_back(curr);
            curr = 0;
        }
    }
    if (curr != 0) consecutive_b.push_back(curr);
    unordered_map<int, int> cnta, cntb;
    for (int i : consecutive_a)
    {
        for (int l = 1; l <= i; l ++)
        {
            cnta[l] += (i - l + 1);

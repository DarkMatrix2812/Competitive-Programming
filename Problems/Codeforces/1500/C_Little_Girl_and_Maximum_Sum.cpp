...
void solve()
{
    ...
    vector<int> freq = prefixArr(tmp);
    vector<pair<int, int>> freq_pos;
    for (int i = 1; i < n + 1; i ++)
    {
        freq_pos.push_back({freq[i + 1], i});
    }
    sort(freq_pos.rbegin(), freq_pos.rend());
    for (auto &p : freq_pos)
    {
        ans[p.second] = pq.top();
        pq.pop();
    ...
    for (auto &p : queries)
    {
        s += (pref[p.second] - pref[p.first - 1]);
    }
    cout << s;
}
int32_t main() 

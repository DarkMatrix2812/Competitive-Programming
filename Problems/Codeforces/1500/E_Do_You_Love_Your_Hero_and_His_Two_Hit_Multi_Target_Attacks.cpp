...
void solve()
{
    int k;
    cin >> k;
    //binary search for l * (l - 1) / 2 <= k
    // take rem = k - l * (l - 1) / 2
    // then binary search again on r * (r - 1) / 2 <= rem
    // then rem2 = rem - r * (r - 1) / 2
    // then place the rem2 in specific spots
    vector<pair<int, int>> ans;
    int low = 0; int high = 500; int l = 0;
    while (low <= high)
    {
        int mid = (low + high) / 2;
        if (mid * (mid - 1) / 2 <= k)

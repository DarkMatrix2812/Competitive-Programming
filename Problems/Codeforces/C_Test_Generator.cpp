...
void solve()
{
    ...
    {
        if (m & (1LL << mask)) contri.push_back(1LL << mask);
    }
    ...
    int low = 0; int high = 1e18; int ans = -1;
    while (low <= high)
    {
        int mid = low + (high - low) / 2;
        bool ok = true;
        if (mid < (s + m - 1) / m) ok = false;
        for (int mask = 0; mask < 63; mask ++)
        {
            if (m & (1LL << mask))
            {
                int x = s % (1LL << mask); // lower bits of s
                int y = m % (1LL << mask); // lower bits of m
                if (y != 0)
                {
                    if (mid < (x + y - 1) / y) ok = false;
                }
            }
        }
        if (ok) 

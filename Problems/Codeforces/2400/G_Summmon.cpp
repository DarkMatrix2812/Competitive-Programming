...
void solve()
{
    ...
    for (int i = 0; i < n; i ++)
    {
        int low = i + 1; int high = n - 1; int j = n;
        while (low <= high)
        {
            int mid = low + (high - low) / 2;
            if (query_gcd(i, mid) < a[i])
            {
                j = mid;
                high = mid - 1;
            }
        ...
        ans += (n - j) * min(a[j] % a[i], a[i] - a[j] % a[i]); // if the gcd == a[i] it contributes 0 anyways, we can make all elements in [l, r] equal
    }
    cout << ans << endl;
}
int32_t main() 

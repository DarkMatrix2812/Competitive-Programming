...
void solve()
{
    ...
    for (int i = 0; i < q; i ++)
    {
        ...
        if (o == 1)
        {
            auto it = upper_bound(a.begin(), a.end(), vala[x - 1]);
            int idx = it - a.begin() - 1;
            S = (S * modinv(min(a[idx], b[idx]), MOD2)) % MOD2;
            S = (S * min(a[idx] + 1, b[idx])) % MOD2;
            a[idx] += 1;
            vala[x - 1] += 1;
        }
        else
        {
            auto it = upper_bound(b.begin(), b.end(), valb[x - 1]);
            int idx = it - b.begin() - 1;
            S = (S * modinv(min(a[idx], b[idx]), MOD2)) % MOD2;
            S = (S * min(b[idx] + 1, a[idx])) % MOD2;
            b[idx] += 1;
            valb[x - 1] += 1;
        }
        cout << S << " ";

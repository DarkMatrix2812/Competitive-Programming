...
void solve()
{
    ...
    // choose n - 1 out of m
    // choose any out of n - 2 to duplicate
    // remaining n - 3 elements (excluding max. and the 2 duplicates - they must be one on each side) can be to either side of maximum
    cout << ((nCr(m, n - 1, MOD2) * (n - 2)) % MOD2) * binExp(2, n - 3, MOD2) % MOD2 << endl;  
}
int32_t main() 
{

...
// // clear k-th bit
// x & ~(1LL << k);
bool check(int x, string s, string t, vector<int> pref, vector<int> suff)
{
    if (x == 0) return true;
    int r = x - 1;
    for (int l = 0; r < s.length(); l ++, r ++)
    {
        if ((l == 0 ? 0 : pref[l - 1]) + (r == s.length() - 1 ? 0 : suff[r + 1]) >= t.length()) return true;
    }
    return false;
}
void solve()
{
    string s, t;
    cin >> s >> t;
    int low = 0; int high = s.length(); int ans = 0;
    vector<int> pref(s.length());
    int curr = 0;
    for (int i = 0; i < s.length(); i++)
    {
        if (curr < t.length() && s[i] == t[curr]) curr += 1;
        pref[i] = curr;
    }
    ...
    {
        if (curr >= 0 && s[i] == t[curr]) curr -= 1;
        suff[i] = t.length() - 1 - curr;
    }
    while (low <= high)
    {
        int mid = low + (high - low) / 2;
        if (check(mid, s, t, pref, suff))
        {
            ans = mid;
            low = mid + 1;
        }

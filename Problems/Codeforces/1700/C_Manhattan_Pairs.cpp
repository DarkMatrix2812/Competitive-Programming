...
void solve()
{
    ...
    for (int i = n / 2; i < n; i ++)
    {
        xu.insert(x[i].second);
        yu.insert(y[i].second);
    }
    vector<int> bucket1, bucket2, bucket3, bucket4;
    for (int i = 1; i <= n; i ++)
    {
        if (xu.count(i) && yu.count(i)) bucket1.push_back(i);
        else if (xl.count(i) && yu.count(i)) bucket2.push_back(i);
        else if (xl.count(i) && yl.count(i)) bucket3.push_back(i);
        else if (xu.count(i) && yl.count(i)) bucket4.push_back(i);
    }
    for (int i = 0; i < bucket1.size(); i ++)
    {
        cout << bucket1[i] << " " << bucket3[i] << endl;
    }
    for (int i = 0; i < bucket2.size(); i ++)
    {
        cout << bucket2[i] << " " << bucket4[i] << endl;
    }
}
int32_t main() 
{

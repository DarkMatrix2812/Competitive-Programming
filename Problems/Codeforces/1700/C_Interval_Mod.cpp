...
void solve()
{
    ...
    for (int i = 0; i < n; i ++)
    {
        ...
        divp[i] = a[i] % p;
        divqp[i] = (a[i] % q) % p;
        mn[i] = min(a[i] % p, (a[i] % q) % p);
    }
    vector<int> pref_p = prefixArr(divp);
    vector<int> pref_qp = prefixArr(divqp);
    vector<int> pref_mn = prefixArr(mn);
    int ans = min({pref_qp.back(), pref_p.back(), accumulate(a.begin(), a.end(), 0LL)});
    for (int i = 0; i <= n - k; i ++)
    {
        ans = min(ans, min(pref_p[i + k] - pref_p[i], pref_qp[i + k] - pref_qp[i]) + pref_mn.back() - (pref_mn[i + k] - pref_mn[i]));
    }
    cout << ans << endl;
}
int32_t main() 

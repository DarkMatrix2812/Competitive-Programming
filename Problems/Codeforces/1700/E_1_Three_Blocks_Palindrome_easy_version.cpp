...
void solve()
{
    ...
    // find potential 'a' candidates first? then check for 'b'? 
    // 'b' being the most frequent element sandwiched between the two 'a' blocks
    vector<vector<int32_t>> freq(201, vector<int32_t>(n + 1, 0));
    for (int i = 1; i <= 200; i ++)
    {
        for (int idx = 1; idx <= n; idx ++)
        {
            freq[i][idx] = freq[i][idx - 1] + (a[idx] == i);
        }
    }
    int ans = 0;
    for (auto &p : indices)
    {
        ...
        int l = 0; int r = p.second.size() - 1;
        while (l < r)
        {
            for (int i = 1; i <= 200; i ++)
            {
                ans = max(ans, 2 * (l + 1) + freq[i][p.second[r] - 1] - freq[i][p.second[l]]);
            }
            l += 1;
            r -= 1;
        }

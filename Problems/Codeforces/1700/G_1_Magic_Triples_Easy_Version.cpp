...
void solve()
{
    ...
    for (auto &p : mp)
    {
        for (int b = 2; b * b * p.first <= 1e6; b ++)
        {
            if (mp.count(p.first * b) && mp.count(p.first * b * b)) ans += (p.second * mp[p.first * b] * mp[p.first * b * b]);
        }
    }
    for (auto &p : mp)
    {
        if (p.second >= 3) ans += (p.second * (p.second - 1) * (p.second - 2));
    }
    cout << ans << endl;
}
int32_t main() 

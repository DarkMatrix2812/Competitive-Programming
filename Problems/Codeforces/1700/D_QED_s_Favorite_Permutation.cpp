...
void solve()
{
    ...
    string s;
    cin >> s;
    vector<bool> shift(n + 1, false);
    vector<int> prefmx(n + 1);
    vector<bool> bad(n + 1, false);
    prefmx[1] = p[1];
    for (int i = 2; i <= n; i ++)
    {
        prefmx[i] = max(prefmx[i - 1], p[i]);
    }
    for (int i = 1; i <= n ; i ++)
    {
        if (prefmx[i] > i) shift[i] = true;
    }
    for (int i = 1; i < n; i ++)
    {
        if (s[i - 1] == 'L' && s[i] == 'R') bad[i] = true;
    }
    int cnt = 0;
    for (int i = 1; i <= n; i ++)
    {
        if (bad[i] && shift[i]) cnt += 1;
    }
    for (int i = 0; i < q; i ++)
    {
        int x;
        cin >> x;
        // we obviously need a O(1) check
        // a LR block prevents transition
        if (shift[x] && s[x - 1] == 'L' && s[x] == 'R') cnt -= 1;
        if (shift[x - 1] && s[x - 2] == 'L' && s[x - 1] == 'R') cnt -= 1;
        s[x - 1] = (s[x - 1] == 'L' ? 'R' : 'L');
        if (shift[x] && s[x - 1] == 'L' && s[x] == 'R') cnt += 1;
        if (shift[x - 1] && s[x - 2] == 'L' && s[x - 1] == 'R') cnt += 1;

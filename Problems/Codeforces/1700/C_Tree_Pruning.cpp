...
void solve()
{
    ...
    p.assign(n + 1, -1);
    vis.assign(n + 1, false);
    // no. of nodes whose depth <= x and whose max child depth is >= x will surivive
    for (int i = 1; i <= n - 1; i ++)
    {
        int u, v;
        cin >> u >> v;
    ...
    vector<int> child_depth(n + 1, -1);
    for (int node = 1; node <= n; node ++)
    {
        child_depth[node] = d[node];
    }
    for (int i = 0; i < order.size() - 1; i ++)
    {
        int node = order[i];
        int par = p[node];
        child_depth[par] = max(child_depth[par], child_depth[node]);
    }
    unordered_map<int, int> cnt_depth;
    unordered_map<int, int> cnt_child_depth;
    for (int node = 1; node <= n; node ++)
    {
        cnt_depth[d[node]] += 1;
        cnt_child_depth[child_depth[node]] += 1;
    }
    vector<int> pref_depth(n + 1, 0);
    vector<int> pref_child_depth(n + 1, 0);
    pref_depth[0] = 1;
    pref_child_depth[0] = cnt_child_depth[0];
    for (int i = 1; i <= n; i ++)
    {
        pref_depth[i] = pref_depth[i - 1] + cnt_depth[i];
        pref_child_depth[i] = pref_child_depth[i - 1] + cnt_child_depth[i];
    }
    int ans = INT_MAX;
    for (int x = 0; x <= n; x ++)
    {
        if (x > 0) ans = min(ans, n - (pref_depth[x] - pref_child_depth[x - 1]));
        else ans = min(ans, n - pref_depth[x]);
    }
    cout << ans << endl;
}
int32_t main() 

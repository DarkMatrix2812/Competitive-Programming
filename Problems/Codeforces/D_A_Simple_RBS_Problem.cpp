...
void solve()
{
    ...
    cin >> t;
    int ls, rs, lt, rt;
    ls = count(s.begin(), s.end(), '(');
    rs = count(s.begin(), s.end(), ')');
    lt = count(t.begin(), t.end(), '(');
    rt = count(t.begin(), t.end(), ')');
    if (ls != lt || lt != rt)
    {
        cout << "NO" << endl;
        return;
    ...
    int l = 0;
    while (pairs_s[l] == n - l - 1) 
    {
        p2s += 1;
        l += 1;
    }
    vector<int> pairs_t(n);
    stack<int> st_t;
    for (int i = 0; i < n; i ++) 
    {
        if (t[i] == '(') st_t.push(i);
    ...
    l = 0;
    while (pairs_t[l] == n - l - 1) 
    {
        p2t += 1;
        l += 1;
    }
    if (p1s == p1t && p2s == p2t) cout << "YES" << endl;
    else cout << "NO" << endl;
}
int32_t main() 
{

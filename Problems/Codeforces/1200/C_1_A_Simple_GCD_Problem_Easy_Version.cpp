...
void solve()
{
    ...
    for (int i = 1; i < n - 1; i ++)
    {
        int d1 = gcd(a[i + 1], a[i]);
        int d2 = gcd(a[i], a[i - 1]);
        if (gcd(lcm(d1, d2), a[i + 1]) == gcd(a[i], a[i + 1]) && gcd(lcm(d1, d2), a[i - 1]) == gcd(a[i], a[i - 1]) && lcm(d1, d2) != a[i] && lcm(d1, d2) <= b[i]) ans += 1;
    }
    if (gcd(gcd(a[0], a[1]), a[1]) == gcd(a[0], a[1]) && gcd(a[0], a[1]) != a[0] && gcd(a[0], a[1]) <= b[0]) ans += 1;
    if (gcd(gcd(a[n - 1], a[n - 2]), a[n - 2]) == gcd(a[n - 1], a[n - 2]) && gcd(a[n - 1], a[n - 2]) != a[n - 1] && gcd(a[n - 1], a[n - 2]) <= b[n - 1]) ans += 1;
    cout << ans << endl;
}
int32_t main() 
{

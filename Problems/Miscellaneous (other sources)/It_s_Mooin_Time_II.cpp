...
void solve()
{
    ...
    for (int i = 0; i < n; i ++)
    {
        ...
        if (used.count(a[i])) repeated[i + 1] = true;
        else repeated[i + 1] = false;
        used.insert(a[i]);
    }
    int ans = 0;
    for (auto &p : positions)
    {
        if (p.second.size() >= 2)
        {
            int i = p.second[p.second.size() - 2];
            int j = p.second.back();
            int val = p.first;
            if (repeated[i]) ans += (distinct[i - 1] - 1);
            else ans += distinct[i - 1];
        }
    }
    cout << ans;
}

...
void solve()
{
    ...
    // Taro tries to maximize x - y
    // Jiro tries to minimize x - y => maximize y - x
    vector<vector<int>> dp(n + 1, vector<int>(n + 1, 0)); // dp[l][r] = maximum value of (curr player score - opp player score) in [l, r]
    for (int i = 1; i <= n; i ++)
    {
        dp[i][i] = a[i];
    }

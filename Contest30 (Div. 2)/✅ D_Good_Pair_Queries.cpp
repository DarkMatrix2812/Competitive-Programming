...
void solve()
{
    ...
    string s, t;
    cin >> s >> t;
    vector<int> prefs(n + 1, 0), preft(n + 1, 0), pref2(n + 1, 0);
    for (int i = 1; i <= n; i ++)
    {
        prefs[i] = prefs[i - 1] + (s[i - 1] == '0' && t[i - 1] == '1');
        preft[i] = preft[i - 1] + (t[i - 1] == '0' && s[i - 1] == '1');
        pref2[i] = pref2[i - 1] + (s[i - 1] == t[i - 1]);
    }
    for (int i = 0; i < q; i ++)
    {
        int l, r;
        cin >> l >> r;
        if (abs((prefs[r] - prefs[l - 1]) - (preft[r] - preft[l - 1])) <= (pref2[r] - pref2[l - 1])) cout << "YES" << endl;
        else cout << "NO" << endl;
    }
}
int32_t main() 

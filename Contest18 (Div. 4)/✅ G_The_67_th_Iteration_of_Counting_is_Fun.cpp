...
void solve()
{
    ...
    vector<int> pref = prefixArr(freq);
    int ans = 1;
    int t = 0;
    for (int i = 0; i < n; i ++)
    {
        ...
        {
            if (i == 0) t = b[i + 1];
            else if (i == n - 1) t = b[i - 1];
            else t = min(b[i - 1], b[i + 1]); // i > 0 && i < n - 1
            if (t >= b[i]) // not possible
            {
                cout << 0 << endl;
                return;
            }
            else ans = ans * (t == b[i] - 1 ? pref[b[i]] : pref[b[i]] - pref[b[i] - 1]) % MOD;
        }
    }
    cout << ans % MOD << endl;
}

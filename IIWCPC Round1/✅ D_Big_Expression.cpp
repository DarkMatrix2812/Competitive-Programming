...
void solve()
{
    ...
    // amplification score = 2^(n-1) only if p1 < p2 > p3 < p4 ...
    // ans is just 2^(n-1) * no. of such permutations?
    ...
    for (int i = 2; i <= n; i ++)
    {
        for (int j = 1; j <= i; j ++)
        {
            dp[i][j] = (dp[i][j - 1] + dp[i - 1][i - j + 1]) % MOD2;
        }
    }
    int perms = 0;
    for (int i = 1; i <= n; i ++)
    {
        perms = (perms + dp[n][i]) % MOD2;
    }
    cout << (binExp(2, n - 1, MOD2) * perms) % MOD2 << endl;
}
int32_t main() 
{

...
void solve()
{
    ...
    for (int i = 1; i <= n; i ++)
    {
        suff[i] = pref1[i] + pref2[i] - pref3[i];
    }
    vector<int> suff_max(n + 1, INT_MIN);
    suff_max[n - 1] = pref1[n - 1] + pref2[n - 1] - pref3[n - 1];
    for (int i = n - 1; i >= 1; i --)
    {
    ...
    for (int i = 1; i <= n - 2; i ++)
    {
        if (pref1[i] >= pref2[i] + pref3[i]) // valid 1st partition
        {
            // does there exist a j > i
            if (suff_max[i + 1] >= suff[i])
            {
                cout << "YES" << endl;
                return;
            }

...
void solve()
{
    ...
    // what if we delete elements to record all possible states reached by deleting
    // then a o(1) check to see if we can reach k from there?
    vector<int> sz;
    for (int i = 0; i < n;)
    {
        int j = i;
    ...
    int distinct = sz.size();
    int ans = 0;
    int i = 0;
    while (i < sz.size())
    {
        int j = i;
        while (j < sz.size() && sz[j] == sz[i])
        {
            j += 1;
        }
        int len = j - i;

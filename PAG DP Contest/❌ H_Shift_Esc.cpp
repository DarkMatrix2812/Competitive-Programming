...
void solve()
{
    ...
    vector<vector<int>> dp(n, vector<int>(m, LLONG_MAX)); // dp[i][j] = minimum cost to reach (i, j)
    vector<vector<vector<int>>> tmp(n, vector<vector<int>>(m, vector<int>(m, LLONG_MAX))); // tmp[i][j][s] = minimum cost to reach (i, j) if row i is shifted s times
    for (int i = 0; i < n; i ++)
    {
        for (int j = 0; j < m; j ++)
        {
    ...
    for (int s = 0; s < m; s ++)
    {
        tmp[0][0][s] = s * k + a[0][s % m];
        dp[0][0] = min(dp[0][0], tmp[0][0][s]);
        for (int j = 1; j < m; j ++)
        {
            tmp[0][j][s] = tmp[0][j - 1][s] + a[0][(j + s) % m];    
            dp[0][j] = min(dp[0][j], tmp[0][j][s]);
        }
    }
    for (int i = 1; i < n; i ++)
    {
        for (int j = 0; j < m; j ++)
        {
            for (int s = 0; s < m; s ++)
            {
                tmp[i][j][s] = (j == 0 ? dp[i - 1][j] + s * k : min(dp[i - 1][j] + s * k, tmp[i][j - 1][s])) + a[i][(j + s) % m];
                dp[i][j] = min(dp[i][j], tmp[i][j][s]);
            }
        }
    }

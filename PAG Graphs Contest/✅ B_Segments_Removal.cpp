...
void solve()
{
    ...
    while (!s.empty())
    {
        ...
        if (L[id] > 0 && R[id] > 0 && val[L[id]] == val[R[id]])
        {
            s.erase({-len[L[id]], L[id]});
            s.erase({-len[R[id]], R[id]});
            len[L[id]] += len[R[id]];
            R[L[id]] = R[R[id]]; // .next.next ahh
            if (R[R[id]] > 0) L[R[R[id]]] = L[id];
            s.insert({-len[L[id]], L[id]});
        }
    }
    cout << ans;
}

...
void solve()
{
    ...
    for (int node = 1; node < n; node ++)
    {
        if (deg[node] == 1) leaves[d[node] % 2].push_back(node);
    }
    ...
    while (rem > 0)
    {
        ...
        {
            ans.push_back({2, leaves[!par].back()});
            ans.push_back({1, 0});
            rem -= 1;
            deg[p[leaves[!par].back()]] -= 1;
            if (deg[p[leaves[!par].back()]] == 1 && p[leaves[!par].back()] != n) leaves[d[p[leaves[!par].back()]] % 2].push_back(p[leaves[!par].back()]);
            leaves[!par].pop_back();
            par = !par;
        }
    }

...
pair<bool, vector<int>> bfs(int s, char target)
{
    queue<int> q;
    q.push(s);
    vis[s] = true;
    ...
    while(!q.empty())
    {
        ...
        q.pop();
        sz += 1;
        if (state[v] == 0) zeroes.push_back(v);
    ...
    if (!ok) return {false, {}};
    if (ones.size() < zeroes.size()) return {true, ones};
    else return {true, zeroes};
}
pair<bool, vector<int>> ans(char target, int n)
{
    vis.assign(n + 1, false);
    state.assign(n + 1, -1);
    vector<int> res;
    ...
    for (int i = 1; i <= n; i ++)
    {
        ...
        {
            islands.push_back(i);
            pair<bool, vector<int>> x = bfs(i, target);
            if (x.first == false) return {false, {}};
...
void solve()
{
    ...
    pair<bool, vector<int>> ans1 = ans('R', n);
    pair<bool, vector<int>> ans2 = ans('B', n);
    ...
    if (ans1.first && ans2.first)
    {
        ...
        {
            cout << ans1.second.size() << endl;
            for (int node : ans1.second)
            {
                cout << node << " ";
            }
        }
        ...
        {
            cout << ans2.second.size() << endl;
            for (int node : ans2.second)
            {
                cout << node << " ";
            }
        }

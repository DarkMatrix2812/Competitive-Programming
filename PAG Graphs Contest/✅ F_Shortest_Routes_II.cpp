...
void solve()
{
    ...
    cin >> n >> m >> q;
    adjd.assign(n + 1, {});
    vector<vector<int>> distances(n + 1, vector<int>(n + 1, LLONG_MAX));
    for (int i = 1; i <= n; i ++)
    {
        distances[i][i] = 0;
    }
    ...
    for (int k = 1; k <= n; ++k) 
    {
        for (int i = 1; i <= n; ++i) 
        {
            for (int j = 1; j <= n; ++j) 
            {
                if (distances[i][k] == LLONG_MAX || distances[k][j] == LLONG_MAX) continue;
    ...
    for (int i = 0; i < q; i ++)
    {
        int a, b;
        cin >> a >> b;
        cout << (distances[a][b] == LLONG_MAX ? -1 : distances[a][b]) << endl;
    }
}
int32_t main() 
{

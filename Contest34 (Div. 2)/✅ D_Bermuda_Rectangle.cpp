...
void solve()
{
    ...
    {
        pref[i] = pref[i - 1] + (i == 1 ? w[0] : (w[i - 1] - w[i - 2])) * h[i - 1];
    }
    for (int i = 0; i < q; i ++)
    {
        int x, y;
        cin >> x >> y;
        int ans = 0;
        int r = upper_bound(w.begin(), w.end(), x) - w.begin();
        int l = lower_bound(h.begin(), h.end(), y, greater<int>()) - h.begin();
        if (min(l, r) >= 1) ans += w[min(l, r) - 1] * y; // leftmost
        if (l <= r) ans += pref[r] - pref[l]; // in between
        if (r < w.size()) // remaining blocks after r
        {
            int prev = (r > 0 ? w[r - 1] : 0);
            if (x > prev) ans += (x - prev) * min(y, h[r]);
        }
        cout << ans << endl;
    }
}

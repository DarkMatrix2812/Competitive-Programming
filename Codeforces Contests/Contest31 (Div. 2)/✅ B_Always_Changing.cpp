...
void solve()
{
    ...
    // use blocks of ones and zeroes
    // abs(o - z) <= 1 where o is no. of ones and z is no. of zeroes
    int blocko = 0; int blockz = 0;
    int curr = s[0];
    if (curr == '0') blockz += 1;
    else blocko += 1;
    for (int i = 1; i < n; i ++)
    ...
    int ans = n;
    for (int finalz = 0; finalz <= blockz; finalz += 1)
    {
        for (int finalo = finalz - 1; finalo <= finalz + 1; finalo ++)
        {
            if (finalo >= 0 && finalo <= blocko)
            {
                // differences are also alternating so abs(d0 - d1) <= 1
                if (abs((o - finalo) - (z - finalz)) <= 1) ans = min(ans, o + z - (finalo + finalz));
            }
        }
    }
    cout << ans << endl;

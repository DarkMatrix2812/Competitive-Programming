...
void solve()
{
    ...
    string s;
    cin >> s;
    // O(n^2) is not possible so even a O(1) check doesn't help
    // sliding window?
    // (pref0[i] - pref1[j - 1]) % 3 == 1 or 2 for all i, j
    vector<int> pref0(n);
    vector<int> pref1(n);
    pref0[0] = (s[0] == '0' ? 1 : 0);
    pref1[0] = (s[0] == '1' ? 1 : 0);
    for (int i = 1; i < n; i ++)
    {
        pref0[i] = (pref0[i - 1] + (s[i] == '0')) % 3;
        pref1[i] = (pref1[i - 1] + (s[i] == '1')) % 3;
    }
    vector<int> diff(n);
    ...
    for (int i = 0; i < n ; i++)
    {
        diff[i] = (pref0[i] - pref1[i] + 3) % 3;
        freq[diff[i]] += 1;
    }
    int cnt1 = 0; // invalid
    for (auto &p : freq)
    {
        cnt1 += (p.second * (p.second - 1) / 2);
    }
    int curr = 1;
    int cnt2 = 0;
    for (int i = 0; i < n - 1; i ++)
    {
        ...
        else
        {
            if (curr >= 3)
            {
                if (curr % 2 == 1) cnt2 += ((curr - 1) / 2 * (curr - 1) / 2);
                else cnt2 += (curr / 2 * (curr / 2 - 1));
            }
            curr = 1;
        }
    }

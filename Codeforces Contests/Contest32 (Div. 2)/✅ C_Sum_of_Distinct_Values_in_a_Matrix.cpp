...
void solve()
{
    ...
    set<int> onlya, onlyb, both;
    for (int x : a)
    {
        if (b.count(x)) both.insert(x);
        else onlya.insert(x);
    }
    for (int x : b)
    {
        if (!a.count(x)) onlyb.insert(x);
    }
    vector<int> ans;
    int cnta = 0, cntb = 0, cntboth = 0;
    for (int x : nums)
    {
        if (both.count(x))
        {
        ...
        else if (onlya.count(x))
        {
            if (cnta < n && cntboth < n + m)
            {
                ans.push_back(x);
                cntboth += 1;
                cnta += 1;
    ...
    cout << accumulate(ans.begin(), ans.end(), 0LL) - (cntboth == n + m ? *min_element(ans.begin(), ans.end()) : 0) << endl;
}
int32_t main() 
{

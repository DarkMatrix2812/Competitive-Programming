...
void solve()
{
    ...
    int median = tmp[n / 2];
    vector<int> greater(n), lesser(n), equal(n);
    for (int i = 0; i < n; i ++)
    {
        if (a[i] > median) greater[i] = 1;
        else if (a[i] < median) lesser[i] = 1;
        else equal[i] = 1;
    }
    vector<int> pg = prefixArr(greater);
    vector<int> pl = prefixArr(lesser);
    vector<int> pe = prefixArr(equal);
    // if median present in [i, j] => abs((pg[j] - pg[i - 1]) - (pl[j] - pl[i - 1])) <= pe[j] - pe[i - 1] - 1
    ...
    for (int j = 1; j <= n; j ++)
    {
        for (int i = 0; i < j; i ++)
        {
            ...
            if ((j - i) % 2 == 1 && ans[i] != -1)
            {
                if ((abs((pg[j] - pg[i]) - (pl[j] - pl[i])) <= (pe[j] - pe[i] - 1)) && (pe[j] - pe[i] >= 1)) // one-indexed pref
                {
    ...
    cout << (ans.back() == -1 ? 0 : ans.back()) << endl;
}
int32_t main() 
{

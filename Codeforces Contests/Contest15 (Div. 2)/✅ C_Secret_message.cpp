...
void solve()
{
    ...
    vector<vector<bool>> present(n, vector<bool>(26, false)); // if char c present in column j
    for (int j = 0; j < n; j ++) 
    {
        for (int i = 0; i < k; i ++) 
        {
            present[j][strips[i][j] - 'a'] = true;
        }
    }
    ...
    for (int f : facts)
    {
        ...
        for (int i = 0; i < f; i ++)
        {
            ...
            for (int j = i; j < n; j += f) // jump
            {
                for (int c = 0; c < 26; c ++)
                {
                    intersect[c] = intersect[c] && present[j][c];
                }
            }
            int common = -1;
            for (int c = 0; c < 26; c ++)

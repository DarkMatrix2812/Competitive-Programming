...
void solve()
{
    ...
    for (int i = 0; i < k - 1; i++)
    {
        ...
        if (p[i + 1] > p[i] + 1 && a[p[i + 1] - 1] != a[p[i + 1]]) curr += 1;
        segments.push_back(curr);
    }
    curr = (p[k - 1] < n - 1 && a[n - 1] != x);
    for (int i = p[k - 1] + 1; i < n; i++)
    {
        if (a[i] != a[i - 1]) curr += 1;
    }
    segments.push_back(curr);
    int ans = max(*max_element(segments.begin(), segments.end()), (accumulate(segments.begin(), segments.end(), 0LL) + 1) / 2);
    cout << ans << endl;
}
int32_t main() 
{

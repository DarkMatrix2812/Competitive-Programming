...
void solve()
{
    ...
    cin >> s;
    // s[i] != s[i + 2]
    string cand1(n, '0'), cand2(n, '0'), cand3(n, '0'), cand4(n, '0');
    for (int i = 2; i < n; i += 4)
    {
        cand1[i] = '1';
        if (i < n - 1) cand1[i + 1] = '1';
    }
    for (int i = 0; i < n; i += 4)
    {
        cand2[i] = '1';
        if (i < n - 1) cand2[i + 1] = '1';
    }
    for (int i = 1; i < n; i += 4)
    {
        cand3[i] = '1';
        if (i < n - 1) cand3[i + 1] = '1';
    }
    cand4[0] = '1';
    for (int i = 3; i < n; i += 4)
    {
        cand4[i] = '1';
        if (i < n - 1) cand4[i + 1] = '1';
    }
    vector<string> cand;
    cand.push_back(cand1);
    cand.push_back(cand2);
    cand.push_back(cand3);
    cand.push_back(cand4);
    int ans = 0;
    for (string str : cand)
    {
        int flag = 0;

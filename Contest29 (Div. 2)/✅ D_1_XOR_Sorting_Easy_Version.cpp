...
void solve()
{
    ...
    // positions where a[i] != sort[i] is what we are interested in
    if (a == sorted)
    {
        cout << 0 << endl;
        return;
    ...
    else
    {
        ...
        for (int i = 0; i < n; i ++)
        {
            if (a[i] != sorted[i]) wrong.push_back(i);
        }
        int ans = 0;
        int k = 0;
        for (int w : wrong)
        {
            int l = lower_bound(sorted.begin(), sorted.end(), a[w]) - sorted.begin();
            int r = upper_bound(sorted.begin(), sorted.end(), a[w]) - sorted.begin() - 1;
            if (w < l)
            {
                k = ((w ^ l) == 0 ? 0 : 1LL << (63 - __builtin_clzll(w ^ l)));
            }
            else if (w > r)
            {
                k = ((w ^ r) == 0 ? 0 : 1LL << (63 - __builtin_clzll(w ^ r)));
            }
            ans = max(ans, k);
        }
        cout << ans << endl;

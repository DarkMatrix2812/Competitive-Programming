...
void solve()
{
    ...
    // ans is bounded by m
    // for each row, check if minimum pieces are required to make stability of any row 0
    int ans = m;
    vector<int> pieces;
    for (int row = n; row >= 1; row --)
    {
        ...
        {
            pieces.push_back(a[row][col]);
        }
        sort(pieces.rbegin(), pieces.rend());
        if (pieces.size() > m) pieces.resize(m);
        int s = 0;
        for (int i = 0; i < pieces.size(); i ++) // m largest pieces
        {
            s += pieces[i];
            if (s >= v[row])
            {

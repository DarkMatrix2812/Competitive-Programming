...
void solve()
{
    ...
    for (int i = 0; i < q; i ++)
    {
        char c; int x;
        cin >> c >> x;
        if (cnt[x] > 0) freq.erase(freq.find(cnt[x]));
        if (c == '+') cnt[x] += 1;
        else cnt[x] -= 1;
        if (cnt[x] > 0) freq.insert(cnt[x]);
        ...
        if (it != freq.rend()) third = *it;
        if (first >= 8 || (first >= 6 && second >= 2) || (first >= 4 && second >= 4) || (first >= 4 && second >= 2 && third >= 2)) cout << "YES" << endl;
        else cout << "NO" << endl;
    }
}
int32_t main() 

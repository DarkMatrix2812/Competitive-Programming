...
void solve()
{
    ...
    for (int i = 1; i < n; i ++)
    {
        mx[i] = max(mx[i - 1], pref[i + 1]);
    }
    for (int i = 0; i < m; i ++)
    {
        int M = mx.back();
        if (M >= x[i]) // one cycle enough
        {
            auto it = lower_bound(mx.begin(), mx.end(), x[i]);
            cout << distance(mx.begin(), it) << " ";
        }
        else // M < x[i]
        {
            if (total <= 0) cout << -1 << " ";
            ...
            {
                int cycles = (x[i] - M + total - 1) / total;
                int rem = x[i] - cycles * total;
                auto it = lower_bound(mx.begin(), mx.end(), rem);
                cout << cycles * n + distance(mx.begin(), it) << " ";
            }
        }
    }
    cout << endl;

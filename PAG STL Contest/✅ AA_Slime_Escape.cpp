...
void solve()
{
    ...
    for (int i = k - 1; i >= 1; i --) 
    {
        ...
        if (sum >= 0) 
        {
            left.push_back({sum, mn});
            sum = 0;
            mn = 0;
        }
    }
    if (sum != 0) left.push_back({sum, mn});
    ...
    for (int i = k + 1; i <= n; i++) 
    {
        ...
        if (sum >= 0) 
        {
            right.push_back({sum, mn});
            sum = 0;
            mn = 0;
        }
    }
    if (sum != 0) right.push_back({sum, mn});
    int i = 0, j = 0;
    while (true) 
    {
        bool moved = false;
    ...
    if (i == left.size() || j == right.size()) cout << "YES" << endl;
    else cout << "NO" << endl;
}
int32_t main() 
{

...
void solve()
{
    ...
    for (int i = 0 ; i < n; i ++)
    {
        v[a[i] - 1].push_back({p[i], i});
        v[b[i] - 1].push_back({p[i], i});
    }
    for (int i = 0; i < 3; i ++)
    {
    ...
    for (int i = 0; i < m; i ++)
    {
        cin >> c[i];
        int color = c[i] - 1;
        while (idx[color] < v[color].size() && seen[(v[color][idx[color]]).second])
        {
            idx[color] += 1;
        }
        if (idx[color] == v[color].size())
        {
            cout << -1 << " ";
        }
        else
        {
            cout << (v[color][idx[color]]).first << " ";
            seen[(v[color][idx[color]]).second] = true;
            idx[color] += 1;
        }
    }
}

...
void solve()
{
    ...
    for (int x = n; x >= 1; x --) 
    {
        ...
        int r1 = *it; // pos of first greater on right
        it++;
        int r2 = *it; // pos second greater on right
        it = s.lower_bound(p);
        it--;
        int l1 = *it; // pos of first greater on left
        it--;
        int l2 = *it; // pos of second greater on left
        ans += x * (p - l1) * (r2 - r1); // how many subarrays have x as 2nd maximum with maximum on right side
        ans += x * (l1 - l2) * (r1 - p);// how many subarrays have x as 2nd maximum with maximum on left side
        s.insert(p);
    }
    cout << ans << endl;
}
